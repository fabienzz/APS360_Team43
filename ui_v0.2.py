...
class LatexUI:
    def __init__(self, root):
        self.root = root
        self.root.title("Latex Model UI")
        ...
        self.min_length_label.grid(row=0, column=3, padx=10, pady=10, sticky="w")

        self.min_length_entry = tk.Entry(root, font=("Arial", 12))
        self.min_length_entry.insert(0, '5')  # Default min length
        self.min_length_entry.grid(row=0, column=4, padx=10, pady=10, sticky="w")

        self.max_length_label = tk.Label(root, text="Max Length:", font=("Arial", 14))
        self.max_length_label.grid(row=1, column=3, padx=10, pady=10, sticky="w")

        self.max_length_entry = tk.Entry(root, font=("Arial", 12))
        self.max_length_entry.insert(0, '20')  # Default max length
        self.max_length_entry.grid(row=1, column=4, padx=10, pady=10, sticky="w")

        self.output_container = tk.Frame(root)
        self.output_container.grid(row=2, column=0, columnspan=5, padx=10, pady=10)
    ...
    def process_latex(self):
        ...
        max_length = self.max_length_entry.get().strip()

        if not input_latex:
            input_latex = ""

        try:
            outputs = latex_model.generate(input_latex, min_length, max_length)
            if len(outputs) != 5:
